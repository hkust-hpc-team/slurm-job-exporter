...
def cgroup_gpus(job_dir, cgroups):
    """Get GPU information for a job's cgroup using nvidia-smi directly"""
    if cgroups == 1:
    ...
    try:
        # Write current process PID to task file
        with open(task_file, 'w') as f:
            f.write(str(os.getpid()) + '\n')

        # Run nvidia-smi -L
        try:
            res = subprocess.check_output(["nvidia-smi", "-L"]).strip().decode()
